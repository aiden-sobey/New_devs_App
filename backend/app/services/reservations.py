...
async def get_property_timezone(property_id: str, tenant_id: str, session=None) -> str:
    """
    Looks up the timezone for a property from the database.
...
async def calculate_monthly_revenue(property_id: str, tenant_id: str, month: int, year: int, db_session=None) -> Decimal:
    ...
    """
    # Look up property timezone and build timezone-aware boundaries
    tz_name = await get_property_timezone(property_id, tenant_id, db_session)
    ...
    # Convert to UTC for querying against UTC-stored timestamps
    start_date_utc = start_date.astimezone(pytz.utc)
    end_date_utc = end_date.astimezone(pytz.utc)

    print(f"DEBUG: Querying revenue for {property_id} from {start_date_utc} to {end_date_utc} (tz: {tz_name})")
...
async def calculate_total_revenue(property_id: str, tenant_id: str) -> Dict[str, Any]:
    ...
    """
    try:
        # Use the shared database pool singleton
